...
class Hj_PageController extends AbstractController
{
    ...
    public function pageElementDetailAction()
    {
        ...
        if($PermissionCheck['return'])
        {
            ...
            elseif(in_array($elementInfo['element_type'] ,[ "clubInfo","clubPermission","clubMemberList","clubMemberLog"]))
            {
                //获取俱乐部列表
                $clubList = (new Hj_Club())->getClubList(['company_id'=>$pageInfo['company_id']],"club_id,club_name");
            }
            elseif(in_array($elementInfo['element_type'] ,[ "activityApply","activityInfo","activityUpdate","managedActivityList"]))
            {
                //获取活动列表
    ...
    public function pageElementDetailUpdateAction()
    {
        ...
        elseif(in_array($elementDetail['element_type'],['activityInfo','activityUpdate',"managedActivityList"]))
        {
            if($detail['id_from']=="from_params")
            {
                $elementDetail['detail']['from_params'] = $detail['from_params'];
