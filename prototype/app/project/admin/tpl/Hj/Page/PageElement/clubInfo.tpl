...
<tr class="hover">
<td>对应俱乐部：</td>
	<td align="left">		<input type="radio" name="detail[id_from]" id="detail[id_from]" value="from_id" {tpl:if(isset($elementInfo.detail.club_id))}checked{/tpl:if}
		 />
		<select name="detail[club_id]"  id="detail[club_id]" size="1">
			{tpl:loop $clubList  $clubInfo}
			<option value="{tpl:$clubInfo.club_id/}"{tpl:if($clubInfo.club_id==$elementInfo.detail.club_id)}selected="selected"{/tpl:if} >{tpl:$clubInfo.club_name/}</option>
			{/tpl:loop}
		</select>
    </td>
</tr>
