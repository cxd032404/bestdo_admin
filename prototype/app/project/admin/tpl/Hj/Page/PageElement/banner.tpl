...
<td>banner类型：</td>
	<td align="left">
		<select name="detail[banner_type]"  id="detail[activity_id]" size="1">
			{tpl:loop $bannerTypeList  $bannerType $bannerName}
			<option value="{tpl:$bannerType/}"{tpl:if($bannerType==$elementInfo.detail.banner_type)}selected="selected"{/tpl:if} >{tpl:$bannerName/}</option>
			{/tpl:loop}
		</select>
    </td>
</tr>
