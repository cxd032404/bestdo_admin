...
{tpl:loop $atheleteList $athlete_id $athlete_info}
  <tr class="hover">
    ...
    <td align="center">{tpl:$athlete_info.athlete_name/}</td>
      <td align="center">{tpl:if($athlete_info.group_id==0)}未分组{tpl:else}{tpl:$athlete_info.group_id/}{/tpl:if}</td>
    ...
    <td align="center">{tpl:$athlete_info.update_time/}</td>
      ...
      <td align="center"><a class = "pb_btn_grey_1" href="javascript:;" onclick="raceDelete('{tpl:$RaceInfo.race_id/}','{tpl:$RaceInfo.race_name/}')">删除</a><a class = "pb_btn_light_1" href="javascript:;" onclick="raceModify('{tpl:$RaceInfo.race_id/}');">修改</a>
          <a class = "pb_btn_grey_1" href="{tpl:$this.sign/}&ac=member.list&race_id={tpl:$RaceInfo.race_id/}">{tpl:if($RaceInfo.athlete==1)}团队列表{tpl:else}选手列表{/tpl:if}</a></td>
  </tr>
{/tpl:loop}
</table>
</fieldset>
